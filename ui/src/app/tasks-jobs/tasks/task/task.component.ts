...
export class TaskComponent implements OnInit {
  ...
  getSchedules(): void {
    this.scheduleService.getSchedulesByTask(this.task.name).subscribe(
      ...
      error => {
        this.notificationService.error($localize`:@@notify.error.title:An error occured`, error);
        this.loadingSchedules = false;
      }
    );
  }
  ...
  openLog(execution: TaskExecution): void {
    this.logModal.open($localize`:@@notify.app-task.title1:Log task execution ` + `${execution.executionId}`, execution);
  }
