import {Component, OnDestroy, OnInit, Renderer2, ViewChild, ViewEncapsulation, HostListener} from '@angular/core';
...
export class StreamFloCreateComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.initSubject.pipe(takeUntil(this.ngUnsubscribe$)).subscribe();
  }
  ...
  contentAssist(doc: CodeMirror.EditorFromTextArea): Promise<any> {
  ...
  markersFromGraphChanged(validationMarkers: Map<string, Flo.Marker[]>): void {
    this.validationMarkers = validationMarkers;
    this.dslEditor.triggerLinting();
  }

  interestingPrefixStart(prefix: string, completions: Array<any>): any {
  ...
  get isCreateStreamsDisabled(): boolean {
    if (this.dsl && this.dsl.trim() && this.contentValidated && this.parseErrors.length === 0) {
      return (
        Array.from(this.validationMarkers.values()).find(
          markers => markers.find(m => m.severity === Flo.Severity.Error) !== undefined
        ) !== undefined
      );
    }
    return true;
