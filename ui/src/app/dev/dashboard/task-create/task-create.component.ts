...
export class TaskCreateComponent {
  ...
  generateTaskName(pattern: string, min: number, max: number): string {
    let random = +min;
    if (min < max) {
  ...
  generateTaskDescription(min: number, max: number): string {
    let random = +min;
    if (min < max) {
      random = Math.floor(Math.random() * (max - min + 1) + min);
    }
  ...
  submit(): void {
    ...
    const observables = Array.from({length: this.model.options.count}).map(() => {
      ...
      return this.taskService.createTask(name, this.model.dsl.value, description).pipe(
        mergeMap(() => {
          if (this.model.options.launch) {
            return this.taskService.launch(name, '', '').pipe(delay(2000));
          }
          return of(name);
        }),
        map(result => {
