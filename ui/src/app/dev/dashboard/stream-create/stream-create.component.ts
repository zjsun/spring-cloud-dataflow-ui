...
export class StreamCreateComponent {
  ...
  generateStreamName(pattern: string, min: number, max: number): string {
    let random = min;
    if (min < max) {
      random = Math.floor(Math.random() * (max - min + 1) + min);
    }
    const dummy = Array(random)
      .fill(0)
  ...
  generateStreamDescription(min: number, max: number): string {
    let random = min;
    if (min < max) {
      random = Math.floor(Math.random() * (max - min + 1) + min);
    }
  ...
  submit(): void {
    ...
    const observables = Array.from({length: this.model.options.count}).map(() => {
      ...
      return this.streamService.createStream(name, this.model.dsl.value, description).pipe(
        mergeMap(() => {
          if (this.model.options.deploy) {
            return this.streamService.deployStream(name).pipe(delay(2000));
          }
          return of(name);
        }),
        map(result => {
  ...
  execute(operations: Array<Observable<any>>): void {
    ...
    operation.pipe(delay(Math.max(this.model.options.delay * 1000, 50))).subscribe(
      () => {
